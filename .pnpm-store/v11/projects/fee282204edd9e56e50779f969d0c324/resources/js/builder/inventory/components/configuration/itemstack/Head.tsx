...
const Head = ({handleChange, currentSlot, updateHead}) => {
    ...
    const copyToClipboard = (text, type) => {
        // @ts-ignore
        window.toast("success", "已复制", `已复制${type}。`, 1000)
                ...
                {showDropdown && (<Dropdown.Menu show style={{
                        ...
                        {heads.length > 0 ? (heads.map((option, index) => (
                                <Dropdown.Item key={index} onClick={() => handleClick(option)}>
                                    <img src={api.getHeadUrl(option.image_name)} alt={'头颅图标'} width={25}
                                         height={25}
                                         className={'me-2'}/>{option.name}
                                </Dropdown.Item>))) : (<Dropdown.Item disabled>未找到选项</Dropdown.Item>)}
                    </Dropdown.Menu>)}

                {head && (<div>
                        <Form.Label>头颅值</Form.Label>
                        <div className="input-group mb-3">
                            ...
                            <button className="btn btn-outline-secondary" type="button" id="button-addon2"
                                    onClick={() => copyToClipboard(head.head_url, '头颅值')}>复制
                            </button>
                        </div>
                        <Form.Label>zHead 数据库<a className={'ms-2'}
                        ...
                        <div className="input-group">
                            ...
                            />
                            <button className="btn btn-outline-secondary" type="button" id="button-addon2"
                                    onClick={() => copyToClipboard(`/zhd search id:${head.url_id}`, '命令')}>复制
                            </button>
                        </div>
                    </div>)}
