...
const InventoryBuilder = () => {
    ...
    // @ts-ignore
    const [data] = useState(() => api.loadEditorContent());
    ...
    const [inventoryContent, setInventoryContent] = useState({
        ...
        slots: Array.from({length: ITEMS_PER_PAGE * MAX_PAGE}, (_, index) => {
            if (data.slots?.[index]) return api.normalizeSlot(data.slots[index], index);
    ...
    const saveData = () => {
        ...
        setTimeout(() => {
            ...
            api.saveEditorState({inventory, slots: inventoryContent.slots}).then(handleResponse).catch(handleResponse);
        }, 350);
    };

    const downloadData = () => {
        api.saveEditorState({inventory, slots: inventoryContent.slots});
        setNeedToUpdate(false);
        api.downloadInventory({inventory, slots: inventoryContent.slots, buttonTypes: data.buttonTypes});
    };

    const importData = async (file) => {
        try {
            const importedContent = await api.importYaml(file);
            setInventory(importedContent.inventory);
            setInventoryContent({currentSlot: -1, slots: importedContent.slots});
            setSlots([]);
            setPage(1);
            setNeedToUpdate(true);
        } catch (error) {
            window.alert(error instanceof Error ? error.message : '无法导入此 YAML 文件。');
        }
    };
                ...
                <div className={'d-flex'}>
                    ...
                    <Inventory inventory={inventory} updateInventory={updateInventory}
                               inventoryContent={inventoryContent}
                                ...
                                onDownload={downloadData} onImport={importData}
